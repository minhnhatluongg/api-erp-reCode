...
namespace API.ERP_Portal_RC.Controllers
{
    ...
    public class EcontractV26Controller : ControllerBase
    {
        ...
        ///   - FilterSaleEmID = "000642"    → chỉ HĐ của nhân viên đó.
        ///   - FilterSaleEmID = "me"        → tự động dùng userCode đang login.
        ///   - CrtUser trống/null           → lấy userCode từ JWT token (mặc định).
        ///   - CrtUser = "012001"           → load cây ASM của NV 012001 (kế toán/quản lý chọn).
        /// </summary>
        [HttpGet("list-paged")]
        public async Task<IActionResult> GetAllPaged([FromQuery] EContractV26Request request)
        {
            try
            {
                var tokenUserCode = User.FindFirst("UserCode")?.Value ?? string.Empty;
                if (string.IsNullOrWhiteSpace(tokenUserCode))
                    return Unauthorized(ApiResponse<object>.ErrorResponse("Không xác định được UserCode từ token.", 401));

                // CrtUser: nếu FE truyền vào (kế toán chọn cây ASM) thì dùng, không thì lấy từ token
                string crtUser = string.IsNullOrWhiteSpace(request.CrtUser)
                    ? tokenUserCode
                    : request.CrtUser.Trim();
                ...
                // "me" → map sang tokenUserCode (không phải CrtUser được truyền)
